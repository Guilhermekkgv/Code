...
function Linux.Create(config)
    ...
    local NotificationHolder = Linux.Instance("Frame", {
        Parent = LinuxUI,
        BackgroundTransparency = 1,
    ...
    local NotificationLayout = Linux.Instance("UIListLayout", {
    ...
    function LinuxLib.Notify(config)
        ...
        local Message = Linux.Instance("TextLabel", {
            Parent = Notification,
            BackgroundTransparency = 1,
        ...
        local tweenIn = TweenService:Create(Notification, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Size = UDim2.new(0, 250, 0, 60)})
        tweenIn:Play()

        spawn(function()
            wait(config.Duration or 5)
            local tweenOut = TweenService:Create(Notification, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Size = UDim2.new(0, 0, 0, 60)})
            tweenOut:Play()
