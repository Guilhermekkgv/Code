    ...
    function LinuxLib.Tab(config)
        ...
        TabBtn.MouseButton1Click:Connect(function()
            for _, tab in pairs(Tabs) do
                ...
                if tab.Icon then
                    tab.Icon.ImageColor3 = Linux.Theme.Text
                end
            end
        ...
        function TabElements.ColorPicker(config)
            ...
            local Preview = Linux.Instance("Frame", {
                Parent = ColorPicker,
            ...
            local Hue = config.Default and config.Default:ToHSV() or 0
            local Saturation = 1
            local Value = 1
            ...
            HueSlider.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    local barSize = HueSlider.AbsoluteSize.X
                    local relativePos = math.clamp((pos - HueSlider.AbsolutePosition.X) / barSize, 0, 1)
                    Hue = relativePos
                    HueKnob.Position = UDim2.new(relativePos, -6, 0, -3)
                    UpdateColor()
                end
            end)
